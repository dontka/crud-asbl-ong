    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: var(--spacing-lg); margin: var(--spacing-xl);">
        ...
        <div class="chart-card" style="background: linear-gradient(135deg, rgba(0, 196, 204, 0.1), rgba(0, 212, 170, 0.05));">
            <div class="chart-content">
                ...
                <h2 style="color: var(--secondary); font-size: var(--font-size-3xl); margin: 0;">€<?php echo number_format($stats['total_salaire_net'] ?? 0, 0, ',', ' '); ?></h2>
            </div>
        </div>
        ...
        <div class="chart-card" style="background: linear-gradient(135deg, rgba(0, 212, 170, 0.1), rgba(255, 107, 157, 0.05));">
            <div class="chart-content">
                ...
                <h2 style="color: var(--success); font-size: var(--font-size-3xl); margin: 0;">€<?php echo number_format($stats['avg_salaire_net'] ?? 0, 0, ',', ' '); ?></h2>
            </div>
        </div>
        ...
        <div class="chart-card" style="background: linear-gradient(135deg, rgba(255, 211, 63, 0.1), rgba(255, 107, 157, 0.05));">
            <div class="chart-content">
                ...
                <h2 style="color: var(--warning); font-size: var(--font-size-3xl); margin: 0;">€<?php echo number_format($stats['total_cotisations'] ?? 0, 0, ',', ' '); ?></h2>
            </div>
        </div>
    </div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <thead style="background: var(--gray-50);">
                        <tr>
                            ...
                            <th style="padding: var(--spacing-md); text-align: right; border-bottom: 2px solid var(--gray-200); font-weight: 600;">Salaire Base</th>
                            ...
                            <th style="padding: var(--spacing-md); text-align: right; border-bottom: 2px solid var(--gray-200); font-weight: 600;">Impôts</th>
                        ...
                        <?php else: ?>
                            <?php foreach ($payrolls as $p): ?>
                                <tr style="border-bottom: 1px solid var(--gray-200);">
                                    ...
                                    <td style="padding: var(--spacing-md); text-align: right;">€<?php echo number_format($p['salaire_base'], 2, ',', ' '); ?></td>
                                    <td style="padding: var(--spacing-md); text-align: right;">€<?php echo number_format(($p['prime'] ?? 0) + ($p['gratification'] ?? 0), 2, ',', ' '); ?></td>
                                    <td style="padding: var(--spacing-md); text-align: right;">€<?php echo number_format($p['cotisation_sociale'], 2, ',', ' '); ?></td>
                                    <td style="padding: var(--spacing-md); text-align: right;">€<?php echo number_format($p['impot_revenu'] ?? 0, 2, ',', ' '); ?></td>
                                    <td style="padding: var(--spacing-md); text-align: right; font-weight: 600; color: var(--success);">€<?php echo number_format($p['salaire_net'], 2, ',', ' '); ?></td>
                                    <td style="padding: var(--spacing-md); text-align: center;">
                                        <span style="padding: 4px 8px; border-radius: var(--border-radius-sm); font-size: var(--font-size-sm);
                                            <?php
                                            $status = $p['statut'] ?? 'brouillon';
                                            if ($status === 'paye') echo 'background: #d4edda; color: #155724;';
                                            elseif ($status === 'valide') echo 'background: #cfe2ff; color: #084298;';
                                    ...
                                    <td style="padding: var(--spacing-md); text-align: center;">
                                        <a href="/hr/payroll/<?php echo $p['id']; ?>/edit" title="Éditer" style="margin-right: 8px; color: var(--primary); cursor: pointer;">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="/hr/payroll/<?php echo $p['id']; ?>/delete" title="Supprimer" style="color: var(--error); cursor: pointer;" onclick="return confirm('Confirmer la suppression ?')">
    ...
    <div class="chart-card">
        ...
        <div class="chart-content" style="text-align: center;">
            ...
            <div style="margin-top: var(--spacing-lg); display: grid; grid-template-columns: 1fr; gap: var(--spacing-md);">
                <?php
                $statusLabels = ['actif' => 'Actif', 'termine' => 'Terminé', 'suspendu' => 'Suspendu'];
    ...
    new Chart(contractStatusCtx, {
        ...
        data: {
            labels: <?php echo json_encode(array_map(function ($k) {
                        $m = ['actif' => 'Actif', 'termine' => 'Terminé', 'suspendu' => 'Suspendu'];
                        return $m[$k] ?? $k;
